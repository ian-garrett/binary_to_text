...
AUTHOR: Ian Garrett
Date = 10/03/2015
ProjectName: Binary to Text

Let's say you have a binary code you want printed in, well, not a bunch of 0s an 1s
Rather than converting all the bytes into decimals, 
then looking those decimals up in a giant table to find the corresponding characters,
and THEN writing down each character to assemble the text version of your binary code, 
you can run this program to create the text version for you! In order, this program:

	1) Creates an array of the bytes included in .txt document given in argument line
	2) Creates a dictionary, with the keys being decimals and the values being their corresponding characters
	3) Calculates the decimal value of each byte and stores them in an array 
	4) Loops through the list of decimal values and assembles a string of the corresponding characters (uses dictionary, key=decimal value=character)

NOTE: I assembled the string of correspondence (rawcodeString) by copying and pasting from http://www.theasciicode.com.ar/ascii-text-characters/at-sign-atpersand-ascii-code-64.html

'''

from sys import argv, exit

#Create binary code string from the binarytoconvert.txt file
binString = (open(argv[1]).read())

#Initialize
print("\n-------BINARY TO text-------\n")

#Display the binary code the user entered in the binarytoconvert.txt file
print("The binary code you provided is:\n"+binString)

#Display length of binary code in bytes (characters)
print("\nThe length of your binary code in bytes (characters) is "+str(int(len(binString.replace(" ", ""))/8)))

binArray = binString.split()

rawcodeString = '''ASCII code 32 = space ( Space )
...
trimcodeString = rawcodeString.split('\n')

codeDict = {};
#Assemble dictionary (key=number, value=char)
i = 0
while i != len(trimcodeString)-1:
	#print(trimcodeString[i].split()[2],trimcodeString[i].split()[4])
	codeDict[int(trimcodeString[i].split()[2])]=trimcodeString[i].split()[4]
	i+=1

numList = []
#Assemble list of decimal values corresponding to bytes
for item in binArray:
	#i is the exponent value, starts at 7 and ends after 0 is used
	i, decimal = 7, 0
	for num in item:
		decimal += int(num)*2**i
		i-=1
...
for num in numList:
	if num in codeDict:
		codeString += str(codeDict[num])

#Display the text version of your binary code
print("\nThe text version of your binary code is:\n"+codeString.replace("space", " ")+"\n"+"\n-------CONVERSION COMPLETE, PROGRAM TERMINATED------\n")